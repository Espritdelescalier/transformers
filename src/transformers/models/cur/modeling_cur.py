...
from .configuration_cur import CURConfig
...
def create_sinusoidal_positions(num_pos: int, dim: int) -> torch.Tensor:
    ...
    sinusoid_inp = torch.einsum(
        "i , j -> i j", torch.arange(num_pos, dtype=torch.float), inv_freq).float()
...
class CURAttention(nn.Module):
    ...
    def forward(
        ...
        if is_torch_fx_proxy(position_ids) or torch.jit.is_tracing():
            ...
            embed_positions = get_embed_positions(
                self.embed_positions, position_ids)
        else:
            embed_positions = self._get_embed_positions(position_ids)

        repeated_position_ids = position_ids.unsqueeze(
            -1).repeat(1, 1, embed_positions.shape[-1])
...
class CURBlock(nn.Module):
    def __init__(self, config):
        ...
        self.attn = CURAttention(config)
        self.mlp = CURMLP(inner_dim, config)
...
class CURPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = CURConfig
    base_model_prefix = "transformer"
    is_parallelizable = True
    supports_gradient_checkpointing = True
    _no_split_modules = ["CURBlock"]
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, CURModel):
            module.gradient_checkpointing = value


CUR_START_DOCSTRING = r"""
    This model is a PyTorch [torch.nn.Module](https://pytorch.org/docs/stable/nn.html#torch.nn.Module) sub-class. Use
    it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage and
    behavior.

    Parameters:
        config ([`CURConfig`]): Model configuration class with all the parameters of the model.
            Initializing with a config file does not load the weights associated with the model, only the
            configuration. Check out the [`~PreTrainedModel.from_pretrained`] method to load the model weights.
"""

CUR_INPUTS_DOCSTRING = r"""
    Args:
...
PARALLELIZE_DOCSTRING = r"""
    ...
    Args:
        device_map (`Dict[int, list]`, optional, defaults to None):
            ...
            have fewer attention modules mapped to it than other devices. For reference, the CUR models have the
    ...
    ```python
    # Here is an example of a device map on a machine with 4 GPUs using cur-6B, which has a total of 28 attention modules:
    model = CURForCausalLM.from_pretrained("EleutherAI/cur-6B")
    device_map = {
        0: [0, 1, 2, 3, 4, 5, 6],
        1: [7, 8, 9, 10, 11, 12, 13],
...
DEPARALLELIZE_DOCSTRING = r"""
    ...
    ```python
    # On a 4 GPU machine with cur-6B:
    model = CURForCausalLM.from_pretrained("EleutherAI/cur-6B")
    device_map = {
        0: [0, 1, 2, 3, 4, 5, 6],
        1: [7, 8, 9, 10, 11, 12, 13],
...
class CURModel(CURPreTrainedModel):
    def __init__(self, config):
...
class CURForCausalLM(CURPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.transformer = CURModel(config)
...
@ add_start_docstrings(
    ...
    [`CURForSequenceClassification`] uses the last token in order to do the classification, as other causal models
...
class CURForSequenceClassification(CURPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class CURForQuestionAnswering(CURPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.transformer = CURModel(config)

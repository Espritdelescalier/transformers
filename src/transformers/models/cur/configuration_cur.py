...
class CURConfig(PretrainedConfig):
    ...
    ```python
    >>> from transformers import CURModel, CURConfig

    >>> # Initializing a CUR 6B configuration
    >>> configuration = CURConfig()

    >>> # Initializing a model from the configuration
    >>> model = CURModel(configuration)
...
class CUROnnxConfig(OnnxConfigWithPast):
    def __init__(
        self,
